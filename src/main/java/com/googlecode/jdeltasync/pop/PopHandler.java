...
import com.googlecode.jdeltasync.IFolder;
import com.googlecode.jdeltasync.IMessage;
...
class PopHandler implements Runnable {
    ...
    private IFolder inbox;
    private List<IMessage> messages;
    private Set<String> deleted = new HashSet<String>();

    public PopHandler(Socket socket, DeltaSyncClient deltaSyncClient, IStore store) throws IOException {
        this.socket = socket;
    ...
    private boolean quit(String line) throws Exception {
        if (!QUIT.matcher(line).matches()) {
            ...
            if (client != null && !deleted.isEmpty()) {
                ...
                client.deleteMessages(getInbox(), getDeletedMessages());
    ...
    private void stat(String line) throws Exception {
        if (!STAT.matcher(line).matches()) {
            ...
            int n = 0;
            long octets = 0;
            for (IMessage msg : getAllMessages()) {
    ...
    private void retr(String line) throws Exception {
        ...
        if (!matcher.matches()) {
            ...
            List<IMessage> msgs = getAllMessages();
            if (n <= 0 || n > msgs.size() || deleted.contains(msgs.get(n - 1).getId())) {
                ...
                client.downloadMessageContent(msgs.get(n - 1), out);
                out.flush();
                writeln("\r\n.");
            }
        }
    ...
    private void dele(String line) throws Exception {
        ...
        if (!matcher.matches()) {
            ...
            if (n <= 0 || n > msgs.size() || deleted.contains(msgs.get(n - 1).getId())) {
                ...
                deleted.add(msgs.get(n - 1).getId());
                writeln(OK);
            }
        }
    }
    ...
    private void uidl(String line) throws Exception {
        ...
        if (!matcher.matches()) {
            ...
            if (n <= 0 || n > msgs.size() || deleted.contains(msgs.get(n - 1).getId())) {
                ...
                writeln(OK_UIDL, n, msgs.get(n - 1).getId());
            }
        } else {
            List<IMessage> msgs = getAllMessages();
            writeln(OK);
    ...
    private void list(String line) throws Exception {
        ...
        if (!matcher.matches()) {
            ...
            if (n <= 0 || n > msgs.size() || deleted.contains(msgs.get(n - 1).getId())) {
                ...
                writeln(OK_STAT, n, msgs.get(n - 1).getSize());
            }
        } else {
            List<IMessage> msgs = getAllMessages();
            writeln(OK);
    ...
    private void folders(String line) throws Exception {
        if (!FOLDERS.matcher(line).matches()) {
            ...
            for (IFolder folder : client.getFoldersCollection()) {
    ...
    private IFolder getInbox() throws Exception {
        if (inbox == null) {
    ...
    private List<IMessage> getAllMessages() throws Exception {
        if (messages == null) {
            messages = new ArrayList<IMessage>(client.getMessagesCollection(getInbox()));
            ...
            logger.info("{} messages in {}", messages.size(), inbox.getName());
        }
        return messages;
    }

    private List<IMessage> getDeletedMessages() throws Exception {
